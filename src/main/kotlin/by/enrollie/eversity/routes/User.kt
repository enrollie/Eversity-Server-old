    ...
    route("/timetable") {
            ...
            return@get when (getUserType(routeUserID)) {
                ...
                UserType.Pupil -> {
                    call.respond(getClassTimetable(getPupilClass(routeUserID).id).let {
                        if (getPupilClass(routeUserID).isSecondShift) TwoShiftsTimetable(
                            monday = Pair(
                                arrayOf(),
                                it.monday
                            ),
        ...
        route("/today") {
            ...
            get("/current") {
                ...
                if (DateTime.now().dayOfWeek == DayOfWeek.SUNDAY.value) return@get call.respond(
                    Json.encodeToJsonElement<Lesson?>(
                        null
                    )
                )
